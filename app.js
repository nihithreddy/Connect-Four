document.addEventListener('DOMContentLoaded',() => {
  ...
  const currentPlayerId = document.querySelector('#player-id');
  var currentPlayer = 1;
  ...
  (function(index){
    ...
    squares[index].onclick = function(){
      ...
      if(squares[index+7].classList.contains('bottom')){
        squares[index].classList.add('bottom');
        if(currentPlayer == 1){
          squares[index].classList.add('player-one');
          //Change the player
          currentPlayer = 2;
          //Show it in the page
          currentPlayerId.textContent = currentPlayer;
        }
    ...
    //if the square below the current square is not  taken, you can't go ontop of it
    else{
      alert('Cant go there try again!');
    }
  }
})(i);
}
...
function checkBoard(){
  //All the Possible Winning Positions of the game
    const winningArrays = [
        ...
        for(var i = 0; i < winningArrays.length; i++) {
            ...
            else if (square1.classList.contains('player-two')&&square2.classList.contains('player-two')&&square3.classList.contains('player-two')
